    ...
    while loopingMainMenu {
        ...
        if let user = readLine(){
            switch user {
                ...
                case "4":
                    //get all of the purchased items to add on the remaining stock
                    itemsInStock = [
                ...
                case "6":
                    let grandTotal = roundedPrice + (roundedPrice * 0.925)
...
func processAddOrRemove(action: Action){
    ...
    switch action {
        case .add:
            if amountInt < itemsInStock[selectedOption]! { //check if the input is less than itemsInStock
        ...
        case .remove:
            if amountInt <= userCart[selectedOption]! { //check if the input is less or equal than the purchased items
            ...
            switch user {
                ...
                guard let selectedOption = getItemInput() else { return }
                    ...
                    if let restock = readLine(), let restockItem = Int(restock) {
                        itemsInStock[selectedOption]! += restockItem
